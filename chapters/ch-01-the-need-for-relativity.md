...
- Explain why the Kennedy–Thorndike experiment constrains length-only explanations of the null results and, together with independent clock tests, supports the linked space-and-time effects of Lorentz invariance.
...
:::{note}
Albert Michelson, a graduate of the U.S. Naval Academy, built his first interferometer in 1881 (in Berlin, with support from Alexander Graham Bell) and refined it with Edward Morley at what is now Case Western Reserve University in 1887. Michelson went on to win the 1907 Nobel Prize in Physics — the first American laureate in the sciences — awarded for his precision optical instruments and the spectroscopic and metrological investigations he carried out with them, a body of work that included this interferometer but was not confined to it, even though the experiment's headline result was a *null* one.
:::
...
:::{dropdown} Why unequal arms provide a different test
...
To first order in the water speed $v$, Fizeau measured something different, and reproducibly so:
...
- **Ether drag.** Perhaps Earth drags the nearby ether along with it, so light near Earth's surface shows no relative motion. The simplest version—complete, uniform drag—conflicts with *stellar aberration*, the small annual shift in the apparent position of stars (about $20.5$ arcseconds, first measured by James Bradley in 1727). In the falling-rain analogy, complete local drag would make the incoming light share Earth's sideways motion and would remove the need to tilt the telescope, contrary to observation. Historically, more elaborate drag models were proposed to recover aberration, so aberration alone did not dispose of every possible ether theory; rather, it forced those theories to become substantially more complicated.
...
- **Emission theories.** Perhaps light simply travels at speed $c$ relative to its *source*, with no ether needed at all — a proposal in the spirit of a bullet's speed depending on the gun that fired it. De Sitter argued in 1913 that a simple version of this theory would badly distort observations of binary stars: light emitted while a star approached Earth would arrive faster than light emitted while it receded, potentially scrambling the apparent orbit. Such distortions are not observed, providing important evidence against simple ballistic theories. The historical argument was not conclusive by itself, because interactions with intervening matter might erase a source-dependent speed before the light reached Earth; laboratory and astronomical tests taken together provide the stronger case that light's vacuum speed is independent of its source.
...
- The Kennedy–Thorndike experiment, using unequal interferometer arms, constrained length-contraction-only explanations. Together with Michelson–Morley and independent clock tests, its null result supports the linked length and time effects of Lorentz invariance.
- The Fizeau experiment, measuring the speed of light in flowing water, found to first order a *partial* drag coefficient, $f = 1-1/n^2$ — neither the full entrainment nor the zero entrainment that simple ether pictures would predict. Relativistic velocity addition produces this coefficient naturally at first order in $v/c$.

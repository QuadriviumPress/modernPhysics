...
test('downloads show verified provenance and never date a mismatched or legacy file', () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(root, 'myst.yml'), 'project:\n  downloads:\n    - file: exports/modern-physics.pdf\n      title: Book\n');
    fs.writeFileSync(path.join(root, 'exports/modern-physics.pdf'), 'pdf fixture');
    fs.writeFileSync(path.join(root, 'exports/metadata.json'), JSON.stringify({
      ...
      files: { 'modern-physics.pdf': createHash('sha256').update('pdf fixture').digest('hex') },
    }));
    ...
    fs.writeFileSync(path.join(root, 'exports/modern-physics.pdf'), 'different export');
    prepareDownloads(root);
    ...
    fs.unlinkSync(path.join(root, 'exports/modern-physics.pdf'));
    prepareDownloads(root);
